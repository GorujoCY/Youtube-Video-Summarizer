...
(Reason: After research, OpenAI is not so generously free, when you register with a new phone number OpenAI grants you specific credits as a trial only that it can then expire in approx. 3 months, May (or may not) remove this option because it is meant to be an accessible type of program, not something you want to pay to use. Making an read only api key does not change it seems.)
